...
const StudentForm = ({
  ...
  const [studentData, setStudentData] = useState<StudentType>({ name: "" });
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      handleClose();
      setError("");
      notify("info", "Student deleted successfully.");
    } catch (err: any) {
      console.log(err);
  ...
  const handleSave = async () => {
    try {
      ...
      isEditing
        ...
        : await axios.post(
            `${process.env.NEXT_PUBLIC_BASE_URL}/api/classes/${classId}/students`,
            {
              name: studentData.name,
            },
            {
      ...
      notify(
        "success",
        `Student ${isEditing ? "changed" : "created"} successfully.`
      );
      setStudentData({
        name: "",
      });
